// SPDX-License-Identifier: MIT
pragma solidity ^0.8.24;

/**
 * @title ITrustGate
 * @notice Hook interface for ERC-8183 contracts to query TRUSTGATE certifications
 * @dev Import this interface directly from https://trustgate.base.eth/.well-known/ITrustGate.sol
 *
 * Usage:
 *   ITrustGate trustgate = ITrustGate(0xYOUR_TRUSTGATE_ADDRESS);
 *   require(trustgate.isTrusted(agent), "Agent not certified");
 */
interface ITrustGate {
    /**
     * @notice Certification levels
     * @dev 0=UNVERIFIED, 1=CONDITIONAL, 2=TRUSTED, 3=FLAGGED
     */
    enum CertLevel {
        UNVERIFIED,
        CONDITIONAL,
        TRUSTED,
        FLAGGED
    }

    /**
     * @notice Get certification level for an agent
     * @param agent Address of the agent
     * @return level Certification level (0-3)
     * @return expiresAt Expiration timestamp
     * @return active True if certification exists and is not expired
     * @dev This is the primary function for hooks to query certification status
     */
    function getCertificationLevel(address agent)
        external
        view
        returns (uint8 level, uint256 expiresAt, bool active);

    /**
     * @notice Check if an agent is trusted
     * @param agent Address of the agent
     * @return True if agent has active TRUSTED certification
     * @dev Shorthand for hooks that only need trust check
     */
    function isTrusted(address agent) external view returns (bool);

    /**
     * @notice Get TRUSTGATE's accuracy score
     * @return evalAccuracy Evaluation accuracy rate (percentage * 100)
     * @return certAccuracy Certification accuracy rate (percentage * 100)
     * @return totalEvals Total evaluations made
     * @return totalCerts Total certifications issued
     * @dev Use this to verify TRUSTGATE's own credibility before trusting its verdicts
     */
    function getAccuracyScore()
        external
        view
        returns (
            uint256 evalAccuracy,
            uint256 certAccuracy,
            uint256 totalEvals,
            uint256 totalCerts
        );

    /**
     * @notice Check if certification is expired
     * @param agent Address of the agent
     * @return True if latest certification is expired
     */
    function isExpired(address agent) external view returns (bool);

    /**
     * @notice Get certification fee
     * @return Fee in USDC (6 decimals)
     */
    function getFee() external view returns (uint256);
}
